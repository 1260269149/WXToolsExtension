...
//  EbiCom
//
//  Created by 张芸休懿 on 2019/9/26.
//  Copyright © 2019 宜蓓康. All rights reserved.
//
...
//图片位置
public enum CHImageButtonMode:NSInteger {
    case imageRight
    case imageLeft
    case imageTop
    case imageBottom
...
class CHButton: UIButton {
   ...
   private var modeDicForState = [String : CHImageButtonMode]()
   private var marginDicForState = [String : CGFloat]()
    
   // 返回当前controlState状态下对应的contentMode属性
   private var buttonContentMode:CHImageButtonMode?  {
      get{
   ...
   //为不同状态设置不同的freeMode
   public func setContentMode(_ mode: CHImageButtonMode?, for state: UIControl.State) {
   ...
   public func set(image: UIImage,title:String,titleColr:UIColor,contentMargin marigin: CGFloat,_ mode: CHImageButtonMode?,for state: UIControl.State) {
